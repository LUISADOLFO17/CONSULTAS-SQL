...
¿Cuales son los 3 principales puertos de los cuales tenemos mayor volumen de importación del producto grado 1 y grado 2 en el 2024?
...
(
        ...
        p2.producto_id = i.producto_id 
        where YEAR(fecha_llegada_importacion) = 2024
        and p2.calidad_producto = 'GRADO 2'
        --group by p.puertos_id, p.nombre_puertos,p2.calidad_producto 
)
...
select p.puertos_id, p.nombre_puertos, sum(volumen_importacion) as sum_importa  from grupo02.Puertos p
inner join grupo02.Importacion i on
i.puertos_id = p.puertos_id 
where YEAR(fecha_llegada_importacion) = 2024
group by p.puertos_id, p.nombre_puertos
order by sum_importa desc; 
...
WITH ParticipacionMercado AS (
    SELECT
        YEAR(e.fecha_pedidos) AS anio,
        DATEPART(QUARTER, e.fecha_pedidos) AS trimestre,
        SUM(e.volumen_pedidos) AS volumen_empresa,
        (SUM(e.volumen_pedidos) / SUM(g.volumen_mercado)) * 100 AS participacion_empresa
    FROM
        grupo02.Pedidos e
    JOIN
        grupo02.Importacion_mercado g ON e.fecha_pedidos = g.fecha_mercado
    WHERE
        YEAR(e.fecha_pedidos) IN (2023, 2024)
    GROUP BY
        YEAR(e.fecha_pedidos), DATEPART(QUARTER, e.fecha_pedidos)
),
